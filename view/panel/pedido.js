/**
 * GESTIÓN DE PEDIDOS - MOCHISTAR
 */

// 1. Cargar la tabla de pedidos al entrar en la sección
async function cargarSeccionPedidos() {
    const contenedor = document.getElementById('contenedor-principal');
    
    // Mostramos un spinner mientras carga (opcional pero recomendado)
    ...
    try {
        const response = await fetch('api/apiPedido.php');
        const json = await response.json(); 

        // VALIDACIÓN: Igual que en tu ejemplo de productos
        ...
        // Extraemos los pedidos de la propiedad 'data'
            ...
            pedidos.forEach(p => {
                ...
                html += `
                    <tr>
                        ...
                        <td><span class="text-muted">ID:</span> ${p.id_usuario}</td>
...
// 2. Función para actualizar el estado (Llamada al POST de la API)
async function actualizarEstadoPedido(id, nuevoEstado) {
    if (!confirm(`¿Confirmas el cambio del pedido #${id} a "${nuevoEstado}"?`)) {
        cargarSeccionPedidos(); // Recargamos para limpiar la selección del select
        return;
    }
    ...
    try {
        ...
        if (res.estado === 'Exito') {
            ...
            cargarSeccionPedidos(); // Refrescar la tabla para ver el cambio
        } else {
